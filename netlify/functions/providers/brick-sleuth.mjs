...
export function parseBrickSleuth(html, pageUrl = BRICK_SLEUTH_URL) {
  ...
  return unique(records.map(item => {
    ...
    return deal({
      source: 'Brick Sleuth',
      setNumber: String(item.set_number),
      name: first(item, ['set_name', 'name', 'title']),
      theme: item.theme,
      price: item.best_price,
      normalPrice: first(item, ['Retail_Price', 'retail_price', 'rrp']) ?? first(analysis, ['retail_price', 'comparison_price']),
      link: url(first(item, ['best_price_url', 'best_price_link', 'product_url', 'affiliate_url', 'url']), pageUrl),
