...
function App(){
  ...
  return <div className="app-shell">
    ...
    {tab==='deals' && <main>
      ...
      <div className="deal-list">{filtered.map(d=><article className={`deal-card ${d.voucher?'voucher-deal':''}`} key={`${d.source}-${d.setNumber}`} onClick={()=>setDetail(d)}><div className="deal-head"><div className="retailer source">{d.source}{d.voucher&&<small className="voucher-badge">VOUCHER{d.voucherCode?` · ${d.voucherCode}`:''}</small>}{d.source==='Argos'&&<small className="stock-badge">STOCK CHECK</small>}</div><span className="updated">{ago(d.updatedAt)}</span></div><div className="deal-main"><div className="set-art"><span>{d.setNumber}</span><b>LEGO</b></div><div className="deal-copy"><span className="set-no">LEGO® {d.setNumber}</span><h3>{d.name}</h3><p>{d.source==='Argos'?'Deal store availability required':d.voucher?'Checkout price — verify voucher':d.primeStatus==='confirmed'?'Prime confirmed':'Delivery unverified'}</p></div><ChevronRight className="chevron" size={20}/></div><div className="price-row"><div><span>{d.voucher?'AFTER VOUCHER':'NOW'}</span><strong>{money(d.price)}</strong></div><div><span>{d.shelfPrice?'SHELF PRICE':'WAS'}</span><s>{money(d.shelfPrice||d.normalPrice)}</s></div><div className="discount">−{d.discount}%</div></div><div className="verdict"><span className={`rating ${d.rating.toLowerCase()}`}>{d.rating}</span><div><b>{d.source}</b><p>{d.rationale}</p></div></div><div className="availability"><Store size={15}/>{d.availability}</div></article>)}</div>
    </main>}
